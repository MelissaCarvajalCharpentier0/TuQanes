...
def default_paths(repo_root: Path) -> dict[str, Path]:
    current = repo_root / "input"
    return {
        "water": current / "water_potability.csv",
        "pool": current / "quantum80_raw.csv",
        "folds": current / "quantum80_folds.csv",
        "pool_manifest": current / "quantum80_manifest.json",
        "best_maps": current / "map_cv_best_by_map.csv",
        "maps_manifest": current / "parts_3_4_manifest.json",
        "classical_grid": current / "quantum80_svm_cv_summary.csv",
    }
...
def prepare_experiment(
    ...
    target_output = output_dir or (root / "output" / config.run_id)
